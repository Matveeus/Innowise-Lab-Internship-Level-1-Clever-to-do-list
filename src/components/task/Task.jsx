...
function Task({ taskId, name, description, checked }) {
  const deleteTask = id => {
      ...
      >
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Typography
            variant="h6"
            component="div"
            sx={{ textDecoration: checked ? 'line-through' : 'none', color: checked ? 'text.secondary' : 'text' }}
            onClick={() => toggleComplete(checked)}
          >
            {name}
          </Typography>
          <Typography
